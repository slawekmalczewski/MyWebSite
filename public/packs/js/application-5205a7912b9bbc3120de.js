...
window.addMarkers = function addMarkers() {
  var element = document.querySelector("#photomarkers-list");
  var photomarkers = window.transactions = JSON.parse(element.dataset.photomarkers);
  map.removeMarkers();
  photomarkers.forEach(function (photomarker) {
    if (photomarker.latitude && photomarker.longitude) {
      var photomarker = map.addMarker({
        lat: photomarker.latitude,
        lng: photomarker.longitude,
        title: photomarker.address
      });
    }
  });
...
document.addEventListener("turbolinks:load", function () {
  ...
  map.addListener("dragend", function () {
    ...
    Turbolinks.visit("/photomarkers?l=".concat(location));
  });
});
...
/******/ });
//# sourceMappingURL=application-5205a7912b9bbc3120de.js.map